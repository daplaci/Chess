...
class BoardManager {
  ...
  move(new_position) {
    ...
    if ((this.hit_piece.name == "king") & this.hit_piece.is_short_castling) {
      if (this.hit_piece.color == "white") {
        for (let enemy_piece of this.black_pieces.all_pieces) {
          //if short castling is checked exit
        ...
        for (let enemy_piece of this.white_pieces.all_pieces) {
          //if short castling is checked exit
          for (let pos of this.hit_piece.short_castle_positions) {
            if (this.is_path_valid(enemy_piece, pos)) {
              return 0;
      ...
      (this.hit_piece.name == "king") &
      this.hit_piece.is_long_castling
    ) {
      if (this.hit_piece.color == "white") {
        for (let enemy_piece of this.black_pieces.all_pieces) {
          //if long castling is checked exit
    ...
    if (
      (this.hit_piece.name == "pawn") &
      (initial_position == this.hit_piece.default_position)
    ) {
      if ((floor(new_position / 8) == 3) | (floor(new_position / 8) == 5)) {
        for (let delta of [1, -1]) {
          if (
            (new_position + (delta % 8) == 0) |
            (new_position + (delta % 8) == 7)
          ) {
            continue;
          }
    ...
    if (
      (this.hit_piece.name == "pawn") &
      (abs(
        floor(this.hit_piece.position / 8) -
          floor(this.hit_piece.default_position / 8)
      ) ==
        6)
    ) {
  ...
  is_king_legal(attacking_color) {
    if (attacking_color == "black") {
      for (let piece of this.white_pieces.all_pieces) {
        // black cannot do a move that puts himself in check
        if (this.is_path_valid(piece, this.black_pieces.king.position)) {
          console.log(
            "invalid move " + piece.name + piece.color + " attacking black king"
          );
          return false;
        }
      }
    ...
    if (attacking_color == "white") {
      for (let piece of this.black_pieces.all_pieces) {
        // white cannot do a move that puts himself in check
        if (this.is_path_valid(piece, this.white_pieces.king.position)) {
          console.log(
            "invalid move" + piece.name + piece.color + "attacking white king"
          );
          return false;
        }
      }
  ...
  is_path_valid(piece, new_location) {
    //check if path for a piece is valid
    if (piece.is_eaten) {
    ...
    if (piece.name == "king") {
      var path = piece.calculate_path_to(
        new_location,
        is_eating,
    ...
    for (let i of path) {
      ...
      if (
        player.get_piece_at_index(i) != 0 ||
        enemy.get_piece_at_index(i) != 0
      ) {
  ...
  commit() {
    ...
    httpDo(api_url, "POST", "json", postData, function (response) {
      console.log(
        "the move was successful and the new fen string has been uploaded" +
          response.move,
        20,
        180
      );

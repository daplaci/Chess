...
class Knight extends Piece {
  ...
  calculate_path_to(position) {
    ...
    if (
      (position == this.position - 6) & (side == "right") ||
...
class King extends Piece {
  ...
  calculate_path_to(
    position,
    is_eating,
    short_castle_right = true,
    long_caste_right = true
  ) {
    if (
      (position == this.short_castle_position) &
      short_castle_right &
      !is_eating
    ) {
      ...
      (position == this.long_castle_position) &
      long_caste_right &
      !is_eating
    ) {
      var start = min(this.position, position);
